import type { ChunkKey, EntityId, LayerId, Vec3, AABB } from '@worldhost/shared';
...
// Solid grid for static collision data
...
// Subscription info for WebSocket clients
...
export class ChunkManager {
  ...
  private devEvents?: { publish: (ev: { type: string; payload?: any; ts?: number }) => void };
  
  // Configuration
  private readonly maxLoadedChunks = 1000;
  private readonly chunkUnloadDelay = 60000; // 1 minute
  // Hard cap for retained chunk metadata to avoid unbounded growth
  private readonly maxRetainedChunks = 20000;

  constructor(ecsWorld: ECSWorld, devEvents?: { publish: (ev: { type: string; payload?: any; ts?: number }) => void }) {
  ...
  getLoadedChunks(): ChunkKey[] {
    return Array.from(this.loadedChunks).map(keyStr => {
      ...
      if (!key) throw new Error(`Invalid chunk key: ${keyStr}`);
      return key;
    });
  }
  ...
  emitSnapshot(chunk: ChunkData, ws: WebSocket): void {
    ...
    const entities: Array<{ id: EntityId; contracts: any[] }> = [];
  ...
  emitDelta(chunk: ChunkData, delta: {
    ...
    if (chunk.subscriptions.size === 0) return;
   ...
   */
  private markChunkModified(chunk: ChunkData): void {
   ...
   */
  initializeSolidGrid(chunk: ChunkData, resolution: number = 16): void {
  ...
  isSolid(chunk: ChunkData, x: number, y: number, z: number): boolean {
    if (!chunk.solidGrid) return false;
   ...
   */
  private generateChunkContent(chunk: ChunkData): void {
    ...
    if (cx % 4 === 0 && cz % 4 === 0) {
      ...
      if (this.ecsWorld.hasEntity(entityId)) return;
      ...
      this.ecsWorld.createEntity(entityId, [
        { type: 'identity', id: entityId, name: `Generated Block (${cx}, ${cz})`, description: 'A procedurally generated block' } as any,
        { type: 'mobility', position: worldPos } as any,
        { type: 'shape', bounds: { min: { x: -0.5, y: -0.5, z: -0.5 }, max: { x: 0.5, y: 0.5, z: 0.5 } }, geometry: 'box' } as any,
        { type: 'visual', color: '#8B4513', visible: true } as any,
        { type: 'solidity', solid: true } as any,
      ]);
  ...
  private cleanupOldChunks(): void {
    ...
    if (this.loadedChunks.size > this.maxLoadedChunks) {
      const loadedEntries: Array<{ keyStr: string; lastAccessed: number; key: ChunkKey }> = [];
      for (const keyStr of this.loadedChunks) {
        const chunk = this.chunks.get(keyStr);
        if (chunk) loadedEntries.push({ keyStr, lastAccessed: chunk.lastAccessed, key: chunk.key });
      }
    ...
    if (this.chunks.size > this.maxRetainedChunks) {
      const candidates: Array<{ keyStr: string; lastAccessed: number }> = [];
   ...
   */
  getLoadedChunkGrids(): Array<{ key: string; grid: SolidGrid }> {
    const results: Array<{ key: string; grid: SolidGrid }> = [];
    for (const keyStr of this.loadedChunks) {
      const chunk = this.chunks.get(keyStr);
