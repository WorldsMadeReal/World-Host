...
export const DurabilitySchema = DurabilityObjectSchema.refine(
  (data) => data.health <= data.maxHealth,
...
export class ContractRegistry {
  ...
  private globalLimits = new Map<string, number>();
   ...
   */
  validate(contract: unknown): { success: true; data: AnyContract } | { success: false; error: string } {
    try {
  ...
  checkLimits(
    ...
    entityContractLimit?: ContractLimit
...
export function createShape(
  bounds: AABB,
  geometry: 'box' | 'sphere' | 'cylinder' | 'mesh' = 'box'
): Shape {
  return {
    type: 'shape',
    bounds,
...
export function createBoxShape(center: Vec3, size: Vec3): Shape {
...
export function createSphereShape(center: Vec3, radius: number): Shape {
  return createShape({
    min: {
      x: center.x - radius,
      y: center.y - radius,
...
export function createSolidity(
  solid: boolean = true,
...
export function createPlayerContracts(
  playerId: EntityId,
  playerName: string,
...
export function createBlockContracts(
  blockId: EntityId,
  position: Vec3,
  color: string = '#8B4513'
): AnyContract[] {
...
export function createItemContracts(
  itemId: EntityId,
  position: Vec3,
  itemName: string,
  color: string = '#FFD700'
...
export function createDoorContracts(
  doorId: EntityId,
  position: Vec3,
  targetLayerId: string,
  targetPosition: Vec3
...
export function validateContract(contract: unknown): { success: true; data: AnyContract } | { success: false; error: string } {
 ...
 */
export function validateContractBoolean(contract: AnyContract): boolean {
  const result = contractRegistry.validate(contract);
  return result.success;
}

export function sanitizeContract(contract: AnyContract): AnyContract {
  ...
  switch (contract.type) {
    ...
    case 'contract_limit':
      const limit = contract as ContractLimit;
      return createContractLimit(limit.limits);
    case 'movement_rules':
      const mr = contract as MovementRules;
      return createMovementRules(mr.stepDistance, mr.allowDiagonal, mr.diagonalNormalized);
    case 'world_conditions':
      const wc = contract as WorldConditions;
      return createWorldConditions(wc.gravity, wc.weather as any, wc.timeOfDay as any, wc.terrainSeed, wc.properties);
    case 'world_commands':
      const wcmd = contract as WorldCommands;
      return createWorldCommands(wcmd.commands);
    case 'command_access':
      const ca = contract as CommandAccess;
 ...
 */
export function createContractLimit(
  limits: Array<{ contractType: string; max: number }>
): ContractLimit {
  return {
...
export function createMovementRules(
  stepDistance: number = 1,
  allowDiagonal: boolean = true,
  diagonalNormalized: boolean = true
): MovementRules {
  return {
...
export function createWorldConditions(
  gravity?: number,
...
export function createWorldCommands(commands: string[]): WorldCommands {
  return {
...
export function createCommandAccess(allowed: string[]): CommandAccess {
  return {
