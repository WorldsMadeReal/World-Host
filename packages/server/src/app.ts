...
import { 
  gameLoopDuration, 
  gameLoopLag, 
  systemUpdateDuration, 
  updateECSMetrics, 
  updateChunkMetrics,
  connectedClients 
} from './metrics.js';
import { TARGET_FPS, TICK_RATE_DISABLED, METRICS_ENABLED, OPEN_VISUALIZER, SERVER_PORT } from './config.js';
...
export function createApp() {
  ...
  (globalThis as any).__DEV_EVENTS__ = devEvents;
...
export async function start(port: number = 8080) {
...
function startGameLoop(context: AppContext) {
