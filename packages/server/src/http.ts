...
import { register as metricsRegister, httpRequests, httpDuration, startTimer } from './metrics.js';
...
<body>
  ...
  fastify.get('/dev/events', async (request, reply) => {
    ...
    // send recent
    for (const ev of context.devEvents.getRecent()) send(ev);
  ...
  fastify.post('/dev/terrain/load', async (request, reply) => {
    try {
      ...
      const filtered = items.filter(i => {
        ...
        if (!m) return false;
        const kcx = parseInt(m[2]!,10); const kcz = parseInt(m[4]!,10);
  ...
  fastify.post('/world/layers', async (request, reply) => {
    try {
      ...
      const layerConfig = context.worldState.createLayer(parsed.data as any);
      reply.code(201);
      return {
        success: true,
        layer: layerConfig,
  ...
  fastify.post('/spawn', async (request, reply) => {
    try {
      ...
      const entityId = context.worldState.spawn(
        body.archetypeId,
        body.layerId,
        body.position,
        body.overrides
  ...
  fastify.post('/save', async (request, reply) => {
    try {
      ...
      const body = parsed.data as z.infer<typeof SaveLoadSchema>;
