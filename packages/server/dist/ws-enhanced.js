...
import { connectedClients, totalConnections, connectionDuration, trackInboundMessage, trackOutboundMessage } from './metrics.js';
export function setupEnhancedWebSocketServer(httpServer, context) {
    ...
    const chunkSubscriptions = new Map(); // ChunkKey -> Set<clientId>
    ...
    // Utility functions
    function cleanupClientSubscriptions(client, subscriptions) {
    ...
    function broadcastToChunkSubscribers(chunkKey, message, excludeClientId) {
...
function handleEnhancedClientMessage(ws, client, message, context, chunkSubscriptions, entityPositions) {
...
function handleSubscribeChunks(ws, client, message, context, chunkSubscriptions) {
...
function ensurePlayer(client, context) {
    if (!client.playerId) {
...
function isCommandAllowed(client, context, command) {
    const ecs = context.worldState.getECSWorld();
    const worldEntities = ecs.getEntitiesWithContract('world_commands');
    if (worldEntities.length > 0) {
        const worldCmds = ecs.getContract(worldEntities[0], 'world_commands');
    ...
    const access = ecs.getContract(client.playerId, 'command_access');
    if (access?.allowed)
        return access.allowed.includes(command);
    return false;
}
function computeViewChunks(layerId, position, radius) {
...
function updateAutoSubscriptions(client, context, chunkSubscriptions) {
    if (!client.playerId || !client.viewRadius)
        return;
    const ecs = context.worldState.getECSWorld();
    const mobility = ecs.getContract(client.playerId, 'mobility');
    if (!mobility)
        return;
    ...
    for (const keyStr of Array.from(client.subscribedChunks)) {
        if (!desiredSet.has(keyStr)) {
            ...
            if (subs) {
                ...
                if (subs.size === 0)
                    chunkSubscriptions.delete(keyStr);
            }
    ...
    for (const keyStr of desiredSet) {
        if (!client.subscribedChunks.has(keyStr)) {
            ...
            if (!subs) {
                subs = new Set();
                chunkSubscriptions.set(keyStr, subs);
            }
            subs.add(client.clientId);
            const key = ChunkUtils.fromString(keyStr);
            if (key)
                sendChunkSnapshot(client.ws, key, context);
        }
    }
}
function handleLoginMessage(ws, client, message, context, chunkSubscriptions) {
    ...
    sendMessage(ws, { type: 'login_ok', playerId: client.playerId, layerId });
}
function handleLogoutMessage(ws, client, context, chunkSubscriptions) {
    ...
    client.subscribedChunks.clear();
    sendMessage(ws, { type: 'logout_ok' });
}
function handleSetViewMessage(ws, client, message, context, chunkSubscriptions) {
    ...
    const mobility = world.getContract(playerId, 'mobility');
    if (!mobility)
        return;
    client.viewRadius = Math.max(0, message.radius);
    updateAutoSubscriptions(client, context, chunkSubscriptions);
    sendMessage(ws, { type: 'set_view_ok', radius: client.viewRadius });
}
function handleMoveDirMessage(ws, client, message, context, chunkSubscriptions, entityPositions) {
    ...
    const playerId = ensurePlayer(client, context);
    const ecs = context.worldState.getECSWorld();
    const mobility = ecs.getContract(playerId, 'mobility');
    if (!mobility)
        return;
    const movementRules = ecs.getContract(playerId, 'movement_rules');
    ...
    try {
        const updated = { ...mobility, position: want };
        ...
        const defaultChunk = { layerId: 'default', cx: 0, cy: 0, cz: 0 };
...
function handleUnsubscribeChunks(ws, client, message, chunkSubscriptions) {
    for (const chunkKey of message.chunkKeys) {
        const chunkKeyStr = ChunkUtils.toString(chunkKey);
...
function sendChunkSnapshot(ws, chunkKey, context) {
    ...
    for (const entityId of allEntities) {
        ...
        entities.push({ id: entityId, contracts: contracts });
    }
    const message = {
        type: 'chunk_snapshot',
        chunkKey,
    ...
    try {
        for (const e of entities) {
            const identity = e.contracts.find(c => c.type === 'identity');
...
function handleMoveMessage(ws, client, message, context, chunkSubscriptions, entityPositions) {
    if (!client.playerId) {
    ...
    if (mobility) {
        const currentPos = mobility.position;
...
function handleAddContractMessage(ws, client, message, context, chunkSubscriptions) {
    try {
...
function handleRemoveContractMessage(ws, client, message, context, chunkSubscriptions) {
    try {
        // Validate that client can modify this entity (for now, only their own player)
        if (message.entityId !== client.playerId) {
            sendMessage(ws, {
...
function broadcastEntityUpdate(entityId, chunkKey, context, chunkSubscriptions, updateType) {
    const chunkKeyStr = ChunkUtils.toString(chunkKey);
    const subscribers = chunkSubscriptions.get(chunkKeyStr);
    ...
    if (updateType === 'entity_despawn') {
        ...
        try {
            context.devEvents.publish({ type: 'entity_despawn', payload: { id: entityId } });
        }
        catch { }
    }
    else {
        ...
        if (updateType === 'entity_spawn') {
            ...
            try {
                const identity = contracts.find(c => c.type === 'identity');
                const kind = (identity?.name || '').toLowerCase().includes('player') ? 'player' : (identity?.name ? identity.name.toLowerCase() : 'entity');
...
//# sourceMappingURL=ws-enhanced.js.map